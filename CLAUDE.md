...
- Every merged PR is a release: `/release [major|minor|patch]` on the branch (SemVer + `CHANGELOG.md` entry; CI checks it). Nothing is tagged; the record of a version is its changelog entry and the commit that raised it. The local build goes to `dist/qr-scanner-generator-X.Y.Z.*` (gitignored); rebuild it after any app change on the branch.
