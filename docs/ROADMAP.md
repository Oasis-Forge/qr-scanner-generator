...
- [ ] **The interstitial's listing sentence, in twenty languages.** The AdMob unit exists (2026-09-22) and `interstitialAdUnitId` in `lib/main.dart` carries it, so ADS-9 is live in a release build. That makes one sentence of the store listing untrue in all twenty languages — "Ads appear only at the bottom of History, Settings and the Create screen". The English source is corrected; the nineteen translations and the CSV need rebuilding before the listing is uploaded again.
