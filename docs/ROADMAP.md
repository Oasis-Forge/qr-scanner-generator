...
- [ ] Accounts: all done bar one — the Play Console app, the upload keystore, AdMob with its consent messages and the support email are all in place (2026-09-21). **Only the Firebase project is missing**, and crash reports wait on it.
  ...
  - AdMob app, consent messages, and both ad units — the banner (2026-09-19) and the interstitial (2026-09-22) — done; **a Firebase project, still missing** (`docs/RELEASING.md`).
...
- [ ] **Re-upload the listing** so the corrected ads sentence reaches Play. The text is done: the AdMob unit exists (2026-09-22), `interstitialAdUnitId` in `lib/main.dart` carries it, and the listing's ads sentence now names the full-screen ad in all twenty languages, with `store/play/store-listing-translations.csv` and `store/play/text/` rebuilt for **Main store listing → Manage translations → bulk import**.
