...
- **PRIV-6** The data-safety form and `docs/privacy-policy.md` are updated in the same PR as any change to permissions, SDKs or network use, and checked against the release manifest before every release (RUN-2).
...
- **SET-1** Theme: System default (the initial choice), Light or Dark, with dynamic colour where the phone supports it.
