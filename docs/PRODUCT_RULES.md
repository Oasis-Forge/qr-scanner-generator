...
- **DATA-8** A successful scan is one that reaches a result screen (RES-1), including Unknown (RES-13) and blocked links (LINK-5); "No code found" isn't one. A successful create is a created code's first render (GEN-13). The Pro prompt (PRO-4) counts these, stored on the device; ads no longer wait on them (ADS-6, dropped 26 September 2026); the review prompt has its own trigger (SET-9).
...
- **ADS-1** Ads appear on three screens only: the History list, the top level of Settings, and the Create type picker. Never on the scanner, any result (a History reopen included), a generator form or created code, the link sheets, Trash, export, backup, batch review, first run, or any dialog. A banner may show from the install's first visit to one of them (ADS-6, dropped 26 September 2026).
...
- ~~**ADS-6** No ad shows before the install's first successful scan or create (DATA-8).~~ **Dropped 26 September 2026:** the gate held the first banner back until a scan or a created code had succeeded, so a new install's first visit to History, Settings or the Create picker held reserved space and never filled it. Banners now run from the first launch on the three screens ADS-1 allows; every other ad rule is unchanged (ADS-1, ADS-5, ADS-7).
...
- **PRIV-1** Consent status is refreshed silently at each start. The consent form shows only where required, and only when an ad screen (ADS-1) is about to request an ad — never at startup, and never on a screen where ADS-1 forbids an ad.
...
12. (26 September 2026) ADS-6 is dropped: banners now run from the install's first launch on the three screens ADS-1 allows, instead of waiting for a first successful scan or create. The gate was reserving ad space on a new install's first visit to History, Settings or the Create picker and never filling it, which is worse than showing the ad it was holding back — an empty reserved strip reads as broken, not as restraint. The consent form (PRIV-1) can now be the first thing a user sees, on first launch, if their first move is one of those three screens; that is the one visible change, and it only ever appears where the law requires it and an ad is actually about to be requested. The working area stays exactly as ad-free as before: ADS-1 itself is untouched, so nothing new appears on the viewfinder, a result, a form or a created code, and the product principle that bends here is the timing of the ad, never its place. This supersedes decision 11's closing clause, which said a Pro owner never sees an interstitial "and neither does anyone before their first scan or created code": the first half still holds, the second no longer does. Decision 11 is left as it was written, as the record of what was decided that day. In practice the interstitial is unaffected either way — it only ever fires after a created code has been saved or shared, and a created code's first render is itself a success under DATA-8, so the gate it lost was one it had already passed.
